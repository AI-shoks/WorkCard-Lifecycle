...
Reviewable IaC для этапа 10. Конфигурация описывает три отдельных GCP project, общий immutable Artifact Registry, IAM, две строго ограниченные GitHub Workload Identity границы, Cloud SQL PostgreSQL 18, Secret Manager, Cloud Run service и отдельные `migrate`/`reset`/`seed`/`verify` jobs, probes, logging/alerts, production PITR, project-level budgets и teardown guards. Она не является разрешением на `apply` и не доказывает hosted readiness.
...
Future orchestration использует отдельный output `deployment_workload_identity.provider` и `deployment_workload_identity.deployer_service_account`. Provider доверяет только тому же immutable repository/owner ID, `main`, `workflow_dispatch` и точному `.github/workflows/deploy.yml`; его `roles/iam.workloadIdentityUser` ведёт только к `work-card-deployer`. Нельзя переиспользовать publisher provider или выдавать deployer JSON key. `deploy.yml` в текущей итерации отсутствует, output нигде не назначен и аутентификация не проверялась в GCP.
...
2. После build/push exact digest — `jobs=true`, `service=false`; затем вручную контролируемый `migrate -> seed -> verify` в чистом staging. `reset` provisioned одновременно, но в bootstrap не запускается. Production `seed` разрешён только для первого bootstrap или отдельно одобренного fixture change.
3. `service=true` только после успешных jobs. Staging квалифицирует digest; production получает тот же digest с pre-traffic checks и ручным promotion.
...
Job execution orchestration, reset schedule/monitor, hosted smoke (включая фактические IAM restore/proxy/socket/logging observations), production approval/traffic switch, rollback/teardown drill и hosted evidence относятся к следующим задачам этапа 10. Эта Terraform-конфигурация ничего не публикует и не создаёт сама по себе; release workflow также остаётся неисполненным до отдельного разрешения на provisioning и manual run.

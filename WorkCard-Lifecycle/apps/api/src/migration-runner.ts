...
async function readMigrations(directory: string): Promise<Migration[]> {
  ...
  if (new Set(versions).size !== versions.length) {
    throw new Error('Версии SQL-миграций должны быть уникальными.');
  }
  return Promise.all(
...
export async function runMigrations(
  ...
  try {
    ...
    // Check the entire applied history before applying anything new or changing grants.
    ...
    for (const applied of history.rows) {
      const file = migrations.find((migration) => migration.version === applied.version);
      if (!file || file.name !== applied.name || file.checksum !== applied.checksum) {
        throw new Error(
          `Уже применённая миграция ${applied.version} была изменена или отсутствует.`,
        );
      }
    }
    const latestVersion = history.rows.at(-1)?.version ?? 0;
    if (
      migrations.some(
        (file) =>
          file.version < latestVersion && !history.rows.some((row) => row.version === file.version),
      )
    ) {

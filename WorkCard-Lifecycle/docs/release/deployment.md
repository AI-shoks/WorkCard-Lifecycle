...
| Runtime DB budgets | fresh runtime connection: transaction/statement/lock/idle `15000/10000/3000/15000` мс; три database-specific role defaults настроены отдельно owner |
...
До первого workflow dispatch каждый Neon target проходит отдельную подготовку: same-image `node dist/migrate.js` создаёт runtime role/schema при закрытом gate, затем owner-only runbook [[database-bootstrap#Neon: настройки timeout для runtime-роли в выбранной БД|настраивает три defaults]] и отдельное свежее runtime-соединение проверяет все четыре бюджета. Hosted наблюдение выявило, что отдельные startup timeout parameters image не стали эффективными в Neon; `transaction_timeout=15000` через существующий `options` применился. Поэтому только `workcard_app IN DATABASE` точной разрешённой БД получает statement/lock/idle `10000/3000/15000` мс. Transaction default, image, URL и exported budgets не меняются. Это предварительная конфигурация окружения, не новая команда owner CLI и не операция внутри действующего workflow; owner readback не заменяет runtime proof.
...
После bootstrap/reset + verify подтвердить сохранность defaults новым runtime-only наблюдением `pg_settings`: ровно четыре уникальные строки с `unit=ms`, transaction/statement/lock/idle строго `15000/10000/3000/15000`. Существующий pool и owner `SET ROLE` не подходят. При расхождении qualification не продолжать, budget не увеличивать. Owner credentials остаются только в отдельном owner процессе; runtime/browser их не получают. Direct URL допускает только `?sslmode=verify-full`, без `PGOPTIONS`, дополнительных URL parameters или TLS overrides.
...
- DB recovery может потерять все visitor actions: новый разрешённый PostgreSQL 18 database target, immutable migrations, отдельная exact role/database timeout configuration, initial seed, verify, новый runtime binding и fresh runtime qualification. Для scoped recovery БД в существующем staging project defaults основной `workcard` не наследуются: после создания роли/schema повторить три owner-only SET для фиксированного имени этой recovery БД и проверить четыре effective budgets. Это не разрешение создавать дополнительные проекты или произвольные БД. Не нужно переносить history или обещать восстановление из короткого Neon PITR окна.

## Первый запуск без предположения о proxy

Одного свободного Render service достаточно для безопасного наблюдения:

1. После разрешённой publication получить exact digest и пройти temporary staging. Initial production owner контейнер того же digest выполняет только `node dist/migrate.js`: runtime role/schema созданы, persistent maintenance gate остаётся закрытым, initial seed ещё не выполнен. Затем отдельная owner-only настройка трёх role/database defaults и fresh runtime budget check по [[database-bootstrap]]; gate и timestamp не менять. Это подготовительные операции до initial bootstrap; они не запускаются API.

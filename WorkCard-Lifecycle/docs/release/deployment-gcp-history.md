---
artifact_id: release.deployment-gcp-history
status: archived
version: 1
owner: release
updated: 2026-09-17
...
> Исторический неактивный GCP runbook, сохранён целиком при переходе 2026-09-17, включая ранее подготовленные foundation/backend изменения. Его commands, next steps, стоимость, статусы аккаунтов и проверки относятся к прежнему проходу и не являются действующей инструкцией либо новой внешней проверкой. Текущий контракт — [[0009-render-free-neon-free-release|ADR-0009]] и [[deployment]].
...
| Reviewable IaC | [Terraform root](../../infra/terraform/README.md) сохраняет full-граф `167/0/0` и отдельно проверенный foundation-граф `136/0/0` с `jobs=false`, `service=false`; `fmt`, `validate`, exact-count/action safety и negative input guards проходят локально, но реального remote plan и provisioning не было |
...
Общий Artifact Registry находится в отдельном release project. Runtime projects имеют только чтение нужного repository; staging и production получают один и тот же digest. Terraform требует конкретные project IDs во всех фазах, а канонический origin окружения — только при его `service=true`; foundation и jobs-only оставляют origin `null`. В Git есть только placeholder example, а реальные значения ещё не назначались.
...
Public interactive остаётся режимом доступа на всём одобренном hosting window; time-box здесь ограничивает срок платных ресурсов, а не заменяет продукт на private demo. До первого provisioning владелец записывает UTC `provisionedAt`, `destroyBy`, максимальную смету, billing contact и ссылку на одобрение. Terraform input `lifetime` fail-closed проверяет эти значения, совпадение currency/contact с budget notifications и добавляет даты в resource labels. Default `destroyBy = provisionedAt + 7 суток`. Одно явное продление допустимо только с отдельной ссылкой на approval, а итоговый срок не может быть позже `provisionedAt + 30 суток`. Отсутствие подтверждённого продления, недоступный billing contact или наступление deadline означает teardown, даже если budget alert не пришёл.

За 24 часа до deadline оператор уведомляет владельца portfolio, проверяет remote-state access и формирует актуальный inventory. Следующие шаги он начинает заблаговременно, чтобы завершить teardown не позднее `destroy_by`:

1. если production service создан, снимает `allUsers roles/run.invoker` по production maintenance runbook и не восстанавливает binding; если service ещё не создан, подтверждает его отсутствие по state и read-only inventory;
2. для существовавшего публичного service подтверждает внешний `403`; во всех фазах проверяет отсутствие активного release run и сохраняет только несекретное release/operational evidence; demo rows не экспортируются как ценные пользовательские данные;
...
4. меняет только локальный input `teardown_mode=true` и строит новый saved plan для **phase A**. Допустимы только снятие `deletion_policy=PREVENT`, `deletion_protection`, SQL `deletion_protection_enabled` и `retain_backups_on_delete`; любой create/replace/delete останавливает phase A. После просмотра конкретного plan получает отдельное явное разрешение на его применение;
5. выполняет одобренный phase-A `terraform apply` именно рассмотренного saved plan, затем сразу строит отдельный `terraform plan -destroy` с теми же reviewed inputs и `teardown_mode=true`;
6. сверяет destroy inventory с фактическим remote state и достигнутой фазой, включая частично завершённый provisioning: пример с одним alert contact содержит 136 ресурсов foundation или 167 полного графа; дополнительные contacts и промежуточная фаза меняют состав. Никакое фиксированное число не заменяет полный просмотр inventory. Затем получает второе, отдельное разрешение на **phase B**. Частичный `-target` teardown запрещён;
...
## Практический чек-лист первого развёртывания и семи суток

На 2026-09-08 пользователь сообщил статус Google Cloud Free Trial `Pending`; на 2026-09-09 его текущий статус не проверялся. Ниже подготовлена будущая последовательность; её пункты не означают разрешение на cloud writes, публикацию image, изменение GitHub settings или commit/push. Установка локальных инструментов отдельно разрешена 2026-09-09: существующий Docker Desktop восстановлен без переустановки, Google Cloud CLI `583.0.0` установлен, его Windows preflight launcher проверен. Аккаунт/ADC пока не настроены. Текущие локальные результаты и ограничения записаны в [[quality-gates]], следующий read-only проход — в [Terraform README](../../infra/terraform/README.md).

### До первого платного ресурса

- [ ] В Billing Overview подтвердить именно активный **Free trial account**, его currency, remaining credit и дату окончания; отличать завершение проверки Pending от **Activate / Upgrade**, которое переводит account в paid billing. Официальный Free Trial предоставляет $300 на 90 дней с ограничениями; фактическая доступность и остаток проверяются у выбранного account. [Google Cloud Free Program](https://docs.cloud.google.com/free/docs/free-cloud-features).
- [ ] Перечитать Git status/diff и получить отдельное разрешение на commit/push нужных изменений. Для будущего release candidate выполнить все семь обязательных CI jobs из [[ci-pipeline]]; старые зелёные runs и локальный build не подтверждают новый SHA.
- [ ] Предоставить существующие Terraform 1.16.x, Node 24, pnpm 11.19.0, `gh` и Google Cloud CLI; получить short-lived ADC без service-account key. Если инструмента нет, его установку согласовать отдельно. Очистить искусственный review token и review-only `TF_VAR_*` перед переходом к реальным credentials.
- [ ] Заполнить ignored `foundation.tfvars` реальными organization/folder, тремя project IDs, billing account, currency, contacts, budgets и lifetime. Не переносить example даты, суммы и `approval_reference` в реальный plan. Если у личного trial account нет organization/folder, текущий contract требует отдельного решения о parent; активация billing этого не устраняет.
- [ ] Подтвердить отдельный существующий GCS state bucket вне трёх управляемых projects, его owner/location/prefix, versioning, uniform access, public access prevention, IAM/audit/lifecycle. Создание отсутствующего bucket — отдельная заранее рассмотренная cloud-операция. State не копировать в Git; backend credentials не записывать в files.
- [ ] Выполнить read-only `check-foundation-prerequisites.mjs` с реальными несекретными аргументами по [Terraform README](../../infra/terraform/README.md). Его PASS дополнить проверкой IAM, quotas, billing currency/eligibility, email verification, inherited bucket IAM и способности backend создавать/снимать lock: скрипт это не доказывает и ничего не записывает.
- [ ] Зафиксировать оператора, billing contact и утверждённую смету по таблице ниже. Начало `provisioned_at` поставить перед первым foundation apply; `destroy_by` — не позже чем через 168 часов и с запасом до фактического окончания trial. Задержка image build или hosted smoke не перезапускает семь суток.
- [ ] Подготовить шесть разных secret payloads из одобренного источника и numeric generation counters. Секреты передавать только ephemeral inputs, исключить echo, shell transcript, Git и artifacts.
- [ ] После отдельного разрешения на backend initialization получить свежий remote foundation plan с locking и обычным refresh. Проверить его JSON как `--phase=foundation`, полный resource/IAM/budget/deletion diff и смету; только затем запросить разрешение на конкретный saved plan. Offline `136/0/0` не подтверждает GCP permissions, API readiness или возможность apply.

### Бюджет до credits и контроль срока

| Поле перед provisioning | Что записать и проверить |
|---|---|
| Общая граница | `lifetime.maximum_cost.amount` и фактическая currency: одобренная сумма **до зачёта credits**, включая три проекта, backend вне stack и резерв завершения |
| Смета | Две Cloud SQL, SSD, backups/PITR, Cloud Run service/jobs, registry/image scan workflow, secrets, logs/metrics/uptime, сеть и GCS state; ставки и дату повторно проверить для `europe-west1` |
| Исходный ориентир | [Оценка foundation от 2026-09-06](../../infra/terraform/README.md): около $4.31 за 168 часов только за две micro DB и 20 GiB SSD; это неполная историческая оценка, не общий лимит и не подтверждённая цена account |
| Три monthly budgets | Утвердить отдельные суммы release/staging/production так, чтобы первый alert оставлял время до общей границы. Example `$10/$30/$30` и lifetime `$10` не согласованы как операционный бюджет и не утверждены |
| Расчёт alerts | `EXCLUDE_ALL_CREDITS`: учитывать usage до promotional и free-tier credits. Текущие правила — actual 50/90/100% и forecast 100%, email channels и billing recipients. Это консервативный сигнал расхода, а не сумма к оплате |
| Общий счётчик | Ежедневно суммировать usage cost всех трёх проектов и state bucket с `provisioned_at`; отдельно записывать credits/net cost. Monthly budgets не объединены в lifetime cap, а переход месяца сбрасывает их период, поэтому общий расход вести через границу месяцев |
| Резерв и stop | До старта утвердить запас на задержку billing reports, teardown, backup/storage и остаточные расходы. При прогнозе выхода за общую сумму, потере contact/reset/alerts или отсутствии оператора остановить продвижение и начать согласование раннего teardown |

Именно описанные в Terraform бюджеты только уведомляют: resource stop/spend-cap automation здесь не настроена. Начальный alert и сведения о расходах могут запаздывать; credits могут уменьшить счёт, но не должны скрывать потребление ресурсов. Обоснование и задержки — [Cloud Billing budgets](https://docs.cloud.google.com/billing/docs/how-to/budgets), поле provider — [google_billing_budget 7.45.0](https://github.com/hashicorp/terraform-provider-google/blob/v7.45.0/website/docs/r/billing_budget.html.markdown#nested_budget_filter). `destroy_by` — организационный deadline и проверка inputs/labels, автоматического удаления по нему нет.

### Последовательность запуска

1. **Foundation**: после одобрения конкретного plan создать только foundation (`jobs=false`, `service=false` в обоих контурах). Сверить outputs/state, бюджеты/почту, SQL/backup настройки, WIF и registry. С этого момента обе БД потребляют ресурсы независимо от Cloud Run.
2. **Образ**: после отдельного разрешения настроить reviewed GitHub Environments/variables из outputs и вручную запустить `release.yml` для текущего CI-qualified `main`. Получить registry digest, scan и manifest; при отказе публикации или scan не переходить к workloads.
3. **Staging bootstrap**: с exact SHA/digest включить только staging jobs, просмотреть новый plan и отдельно разрешить apply. Выполнить `migrate → seed → verify`; затем по отдельно рассмотренной service-фазе создать private staging service с единственным каноническим origin и healthy revision. Production workloads пока выключены. Strict checker имеет только foundation/full contracts; промежуточный plan требует полного ручного review, его нельзя выдавать за прошедший full gate.
4. **Staging qualification**: по отдельному разрешению запустить `deploy.yml` с тем же digest и проверить существующую baseline revision, повторные jobs, hosted lifecycle, IAM/Origin/CSRF, logs/proxy/socket observations и связанные evidence. При провале исправлять новым кандидатом, без retry ради зелёного статуса.
5. **Production**: только после staging evidence отдельно рассмотреть production jobs/service plan, начальный seed, backup/PITR, точный origin, maintenance и promotion того же digest. Провести ограниченный smoke и rollback drill. До публичного доступа назначить оператора daily reset и last-success проверки: автоматического scheduler в IaC нет.

### Семидневное окно от первого foundation provisioning

Все сроки считают от UTC `provisioned_at`, а не от полуночи или первого посетителя. В рабочем журнале достаточно времени, результата, cost/credit counters и ссылок на несекретные execution/revision evidence.

| Интервал | Практические действия и условие продолжения |
|---|---|
| День 1, 0–24 ч | Проверить foundation inventory, реальные budgets/contacts, SQL backups, backend и WIF; пройти image/staging bootstrap в оставшееся окно. До hosted PASS публичный launch не выполнять |
| День 2, 24–48 ч | Завершить staging qualification, отдельно одобрить production promotion и rollback drill; записать точные digest/revisions и первый успешный reset/verify |
| День 3, 48–72 ч | Проверить desktop/mobile основной процесс, permissions/Origin rejection, 5xx/readiness и отсутствие секретных markers в logs; новые действия посетителей остаются synthetic |
| День 4, 72–96 ч | Проверить capacity, sessions, SQL storage/connections, backup/PITR status и gross расход/прогноз; performance не проверять бесконтрольной публичной нагрузкой |
| День 5, 96–120 ч | Проверить ежедневные reset/verify и recovery evidence, общий расход/остаток времени; если критерии не выполнены, не расширять срок молча |
| День 6, 120–144 ч | За 24 часа до deadline связаться с владельцем, проверить доступ к remote state, достигнутую фазу и полный inventory; подготовить teardown review и время оператора |
| День 7, 144–168 ч | Завершить тест, сохранить несекретные результаты; по двум отдельным разрешениям выполнить phase A и phase B из runbook выше до `destroy_by`. При отсутствии разрешения заранее эскалировать владельцу, не обходить deletion guards |
| После teardown | Read-only проверить отсутствие ресурсов/billing linkage и оставшиеся backups, registry/state retention; повторно проверить запаздывающие начисления. Итог записать с фактическими cost/credits и остаточными обязательствами |

Каждые сутки независимо от достигнутой фазы: проверить usage и прогноз, inventory/SQL alerts, backups и оставшееся время. Для уже запущенного production дополнительно проверить health и последнее успешное `reset → verify` (интервал не больше 24 часов); foundation без jobs/service не требует выполнения отсутствующего reset. После 26 часов без reset, failed reset/verify или capacity stop production public binding не возвращать до восстановления по maintenance runbook. Если тест стартовал позже, сокращается доступное тестовое время; новое семидневное окно автоматически не возникает. Продление требует отдельного explicit approval и обновлённой сметы, общий предел остаётся 30 суток.

**Первый шаг после снятия Pending:** открыть Billing Overview выбранного account в режиме просмотра и подтвердить Free trial status, currency, остаток credits и дату окончания; затем передать реальные несекретные inputs для read-only foundation preflight. Активация сама по себе не является разрешением на apply.
...
Первый foundation-вариант с обоими `jobs=false`/`service=false` отдельно даёт `136 create / 0 change / 0 destroy`: три projects, 25 APIs, budgets, registry, IAM/WIF, две Cloud SQL, восемь secret containers/versions, logging и шесть alerts, без Cloud Run workloads, workload IAM, uptime checks, image digest или origins. Negative tests отклоняют workload без release inputs, lifetime более 7 суток без extension approval и подмену foundation/full contract.

Для remote state подготовлены ignored partial-config templates и read-only prerequisite checker. Они требуют заранее существующий отдельный GCS bucket с versioning, uniform access, public access prevention и непубличным IAM; Terraform credentials и secret payloads в files не допускаются. Это всё ещё только структурный review plan: фактический bucket/backend access, GCP hierarchy/billing/project IDs, contacts/lifetime approval и cloud credentials отсутствуют, provider refresh был выключен и `terraform apply` не запускался.

## Что ещё блокирует фактический deployment и закрытие этапа 10

- IaC локально проверен как foundation `136/0/0` и full `167/0/0`, но реальные GCP hierarchy/billing/project IDs, budget/lifetime/contact approval, защищённый pre-existing remote backend, ADC, IAM/quota checks и разрешение на `apply` отсутствуют; ни один cloud resource не создан;
- WIF/registry ещё не созданы; read-only API показывает `0` GitHub Environments, а состояние repository variables недоступно текущей авторизации и этим шагом не менялось. Ручной release workflow не запускался; опубликованный image и фактический manifest отсутствуют;

...
- Текущий Render target: UI подтвердил отдельный account `usr-dalsn92d0e5s738gnrbg` и workspace `tea-dalsn92d0e5s738gnr60`, вход email/password. Workspace пуст; Hobby, без payment method, pending charges и invoices; использование `0/750` instance hours, `0/5 GB` bandwidth и `0/500` pipeline minutes. При первичном UI preflight 2FA была выключена, Git credentials и API keys отсутствовали. Новый `RENDER_OWNER_ID` установлен в GitHub `production`; DPAPI bundle перепривязан к тому же owner с сохранением repository binding и четырёх исходных `SecureString`.
- Render credentials: после UI preflight в новом аккаунте создан постоянный key `workcard-lifecycle-production-deploy`, сохранён отдельным DPAPI `SecureString`; ACL bundle — только текущий Windows пользователь, `SYSTEM`, `Administrators`, наследование выключено. Exact stdin передача в GitHub `production.RENDER_API_KEY` выполнена без вывода значения. Официальный `GET /users` подтвердил `usr-dalsn92d0e5s738gnrbg`, `/owners` вернул ровно один `tea-dalsn92d0e5s738gnr60`, `/services` для exact owner пуст. WorkCard service, Neon и общий $0-профиль эксплуатации ещё не подтверждены.
...
Первый [push CI](https://github.com/AI-shoks/WorkCard-Lifecycle/actions/runs/35208854588) выявил Linux ShellCheck `SC2034` в счётчике ожидания staging readiness. Добавлена диагностика номера попытки без изменения readiness/assertions/deadline. После исправления все семь обязательных jobs прошли в [push CI](https://github.com/AI-shoks/WorkCard-Lifecycle/actions/runs/35210096941) и [PR CI](https://github.com/AI-shoks/WorkCard-Lifecycle/actions/runs/35210101556); [PR #3](https://github.com/AI-shoks/WorkCard-Lifecycle/pull/3) merged в `1195892f15f2f240dd04f39e8388d6bb8802d9a7`. Полный [push CI этого main SHA](https://github.com/AI-shoks/WorkCard-Lifecycle/actions/runs/35210700261) также прошёл все семь jobs; дерево merge совпало с проверенным кандидатом.

[Release workflow](https://github.com/AI-shoks/WorkCard-Lifecycle/actions/runs/35211261430) построил image один раз для `linux/amd64`, опубликовал и повторно скачал exact digest, проверил config/revision/APP_VERSION и Trivy `0 HIGH/CRITICAL`. Resume и повторная сборка не потребовались: anonymous pull прошёл в workflow и отдельно на локальном Docker с пустым config; независимый anonymous GHCR token/manifest запрос вернул HTTP `200` и тот же `Docker-Content-Digest`.

- Source SHA: `1195892f15f2f240dd04f39e8388d6bb8802d9a7`.
- Image: `ghcr.io/ai-shoks/workcard-lifecycle/work-card@sha256:231d91a73a72275cefa0bbfe25d316b73315eb1f783d5618ebf55019a39ff057`.
- Config digest: `sha256:b4a49e231f771826e45525f8ae14b42e8ee3cfe9e568d205b40b185ba89be25e`.
- [Постоянный release record](https://github.com/AI-shoks/WorkCard-Lifecycle/releases/tag/work-card-1195892f15f2f240dd04f39e8388d6bb8802d9a7): `release-record.tar.gz` и `release-record.sha256`, архив SHA-256 `e2edb7672b9b2c4a9a735ece260c601a6d60871aef25f2db4ddf77095f1e0c89`. Локальный `fetch-release.mjs` проверил checksum, schema, scan и исходные CI/release runs. Образ и record сохраняются на весь срок эксплуатации.

Это публикация image, а не работающий deployment. Отдельный Render аккаунт, owner binding, постоянный API key и его безопасная передача подтверждены. Neon login остаётся отдельным prerequisite; новые demo projects/service, реальные DB operations, staging/production hosted qualification ещё не выполнялись. Смена Render аккаунта сохраняет опубликованные SHA/digest и не требует rebuild. Предыдущего совместимого опубликованного image нет; повторный deploy этого же digest не будет считаться rollback. Автоматизированный public smoke отдельно не доказывает TLS/role negatives, reset/cancellation, cold start, clean DB recovery или фактически истёкшие 26 часов; эти проверки остаются отдельными hosted prerequisites.

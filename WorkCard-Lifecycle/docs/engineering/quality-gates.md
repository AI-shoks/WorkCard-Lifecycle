...
Это публикация image, а не работающий deployment. Neon login и постоянный Render API key ожидают ответов на конкретные запросы доступа; новые demo projects/service, реальные DB operations, staging/production hosted qualification ещё не выполнялись. Предыдущего совместимого опубликованного image нет; повторный deploy этого же digest не будет считаться rollback. Автоматизированный public smoke отдельно не доказывает TLS/role negatives, reset/cancellation, cold start, clean DB recovery или фактически истёкшие 26 часов; эти проверки остаются отдельными hosted prerequisites.

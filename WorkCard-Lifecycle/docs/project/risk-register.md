...
| R-015 | Release plan, reviewable IaC или неисполненный workflow будут ошибочно выданы за работающий hosted deployment | средняя | высокое | Явно фиксировать, что plan `167 to add`, workflow code и локальные проверки не равны `apply`/публикации; этап 10 закрывать только по фактическим SHA/digest/revision/smoke evidence |

...
**Скорректирован:** 2026-07-17. Пять UX-документов версии 4 и отдельный 14-шаговый прототип покрывают core sequence `UC-001 → UC-002 → UC-003(first article) → UC-004 → UC-005 → UC-003(serial) → UC-004 → UC-006 → UC-015 → UC-009`; UX явно отделяет per-card `UC-006` от final-batch `UC-015`, показывает prerequisites и read-back actor/time/ID. Свежий structural audit: `48` документов, `0` errors, `0` warnings; semantic/ID/metadata gates, JS syntax и headless interactive checks прошли.
